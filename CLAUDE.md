...
- **A batch is one lock, one projection, per-item transactions — never one transaction.** `decideBatch` validates every item against the settled projection before applying any, then applies in order, each its own journal transaction; a failure never rolls back a sibling. Don't "optimize" it into a single multi-file transaction, and don't push or `onWritten()` per item (both happen once at the end). `dryRun` reuses the real path with the stage's `probe` flag — add a pre-check to the apply path and the dry run gets it for free; add it anywhere else and they drift. Automatic rules ARE this batch (`methodOverride: "automatic"` + a re-check `guard`); keep the guard's checks in sync with what `auto_ready` promised.
