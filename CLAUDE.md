...
- **Removing invalid entries is all-or-nothing.** Because only a valid manifest may be persisted, removing one of two invalid layers is refused — the other still fails validation. `DELETE /api/sources?name=` therefore repeats (`searchParams.getAll`), and the console's Remove on an invalid row names every other invalid row before it sends them together. A single removal that leaves invalid layers behind answers 409 listing what blocked it, never a 500.
