...
| `packages/core/src/search.mjs` | Ranking behind the `search` and `find_captures` tools: BM25F over stemmed tokens, per-field boosts and length normalization, 7-day recency half-life for captures, plus a log-damped static inbound-link prior (`LINK_PRIOR_WEIGHT`/`linkPriorMultiplier`, mirrored bit-identically in `search-index.mjs`/`search-store.mjs` — see [notes/link-prior.md](docs/architecture/notes/link-prior.md)) applied after the best-layer merge. Importable and side-effect-free precisely so the eval can score it |
