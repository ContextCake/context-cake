...
| `packages/core/src/service.mjs` | Embeddable HTTP service: read API, background index, sources CRUD, settings, file APIs, console mount. Background indexing runs through a concurrency-limited queue (`maxConcurrentIndexing` setting) rather than starting every source's pass at once; `POST /api/active-source` lets a client mark which layer is on screen so it jumps the queue |
