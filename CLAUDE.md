...
- All git mutations against a live root go through `git-core.mjs` (advisory `.contextcake.lock`, per-repo serialization) — never call git directly against a live layer from engine code. That includes *file writes* into the live root: a discrepancy decision whose target is inside the live layer stages and renames inside `commitPathsWithMutation` (control/discrepancies.mjs), never through a bare `stageSectionTransaction().commit()`. The Files editor (`writeFileApi`/`writeSectionApi`) still bypasses this — a known, separate follow-up.
