...
- **Rules may wildcard `conceptType`/`key`, never `target`.** A wildcard target is the auto-acknowledge-a-typo failure the target pin exists to prevent; `validateRule` refuses it. A wildcard rule and an exact rule that disagree are a `ruleConflict` (automation off) — do not invent a specificity order.
