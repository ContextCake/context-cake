...
| `packages/core/src/search.mjs` | Ranking behind the `search` and `find_captures` tools: BM25F over stemmed tokens, per-field boosts and length normalization, 7-day recency half-life for captures, a log-damped static inbound-link prior (`LINK_PRIOR_WEIGHT`/`linkPriorMultiplier`, mirrored bit-identically in `search-index.mjs`/`search-store.mjs` — see [notes/link-prior.md](docs/architecture/notes/link-prior.md)) applied after the best-layer merge, and section-level body scoring for concepts (`scoreConceptSections`/`analyzeConceptFields` — a concept's score is the max over its sections, not its whole body — see [notes/section-retrieval.md](docs/architecture/notes/section-retrieval.md)). Importable and side-effect-free precisely so the eval can score it |
| `packages/core/src/search-store.mjs` | SQLite-backed (`node:sqlite`) on-disk BM25F posting store, differential-tested bit-identical to `searchConcepts` and `createSearchIndex`: persists postings so a restart or a freshly spawned stdio MCP process skips the cold re-parse, and keeps no per-document term map in the JS heap. Postings are reused by FILE fingerprint (size/mtime), so any change to what a document parses into must bump `FORMAT_VERSION` or unchanged files keep stale postings. Not yet wired into `service.mjs`/`retained-search.mjs`/`mcp-server.mjs` |
| `packages/core/src/platform-paths.mjs` | Where config, durable data, and cache live per OS (control-plane spec §5.12), with `CONTEXTCAKE_{CONFIG,DATA,CACHE}_DIR`/`CONTEXTCAKE_MANIFEST` overrides. The one answer the app, the app-bundled CLI, the npm CLI, and promotion bindings share. Never hard-code `~/Library` or `~/.config` again |
| `packages/core/src/frontmatter.mjs` | `splitFrontmatter`: the only YAML fence detector, LF or CRLF. Every frontmatter reader and writer goes through it; a local `startsWith("---\n")` silently drops a CRLF file's type/title/dates |
...
| `packages/core/src/control/discrepancies.mjs` | `createDiscrepancyOperations(caps)`: the one memoized discrepancy projection (keyed `corpusKey \| sourceHealthSig \| sidecarRevision`, read by the list/compact/summary/detail routes, the decision guard, the batch, and the automatic-rules job), decisions (`decide`, `decideBatch` — one lock, one projection, per-item transactions, `dryRun` via the stage's `probe`), the broken-link fix actions (`rewrite_link`/`unlink` edit the effective contributor's section read live; `create_stub` writes a new concept through the create-mode transaction), rules, priorities, and recovery. `runAutomaticRules` is that batch with a re-check guard. Capabilities are injected — `corpus` (the host's resolved corpus + status from one pin), `fileRoots`, `selectedLayers`, the stores, `readLiveSection`, `onWritten`, `git: { commitPathsWithMutation, push }`. A decision touching the live team layer stages AND commits inside git-core's repo lock (F30). Why: [notes/discrepancy-projection.md](docs/architecture/notes/discrepancy-projection.md) |
| `packages/core/src/discrepancies.mjs` | Pure projection + wire shapes: `buildDiscrepancies` (records with `candidates`/`bestCandidate` on broken links — structural, never in `revision`; rules match `conceptType`/`key` exactly or as `*`, `target` always exactly), `summarizeDiscrepancies`, `compactDiscrepancy`, `filterDiscrepancies`, `ACTIONABLE_STATUSES`, and the link-text helpers `rewriteLinkTarget`/`removeLink` that walk exactly the patterns `extractLinks` reports |
| `packages/core/src/discrepancy-rules.mjs` | Rule store, shared-rule document (parse/serialize), `validateRule` (actions `prefer_source`/`acknowledge`/`rewrite_link`; `*` legal on `conceptType`/`key`, never on a broken link's `target`), and the suggestion miner (`suggestDiscrepancyRules`: exact patterns from three consistent decisions, plus a `generalized: true` `*` suggestion only when the evidence spans two (conceptType, key) pairs) |
...
- **The manifest is a trust boundary.** An `mcp` layer spawns `command` with `args` from the manifest — a manifest you did not author can run arbitrary commands as your user. Only point `--manifest` at configs you trust (same model as any MCP client config). `CONTEXTCAKE_MANIFEST` and `CONTEXTCAKE_CONFIG_DIR` choose the CLI's default manifest the same way, so an environment you did not set (a repo's direnv file, a harness config's `env`) crosses the same boundary.
...
- All git mutations against a live root go through `git-core.mjs` (advisory `.contextcake.lock`, per-repo serialization) — never call git directly against a live layer from engine code. That includes *file writes* into the live root: every decision that changes content — `POST /api/discrepancy-decisions`, `POST /api/discrepancy-decisions/batch`, automatic rules, the broken-link actions (`rewrite_link`/`unlink`/`create_stub`), and the legacy `POST /api/conflict-resolutions` — goes through `commitDecisionWrite` (control/discrepancies.mjs), which stages and renames inside `commitPathsWithMutation` when a target is in the live layer, never through a bare `stageSectionTransaction().commit()`. The Files editor (`writeFileApi`/`writeSectionApi`, `PUT /api/file` and `/api/section`) still bypasses this — a known, separate follow-up.
- **Every discrepancy read answers from one memoized projection** (`discrepancyOps.project()`), keyed on corpus + source health (+ the aggregate `indexing` flag) + sidecar stat + an in-process write counter. Never call `buildDiscrepancies` over the corpus anywhere else, and if you add a write path for decisions/rules/priorities outside `control/discrepancies.mjs`, call `discrepancyOps.noteWrite()` after it or the memo will serve the pre-write answer for up to one stat granularity. `GET /api/discrepancies` bare envelope is byte-compatible; `?fields=compact`/filters/`?id=`/`/summary` are additive.
- **A batch is one lock, one projection, per-item transactions — never one transaction.** `decideBatch` validates every item against the settled projection before applying any (`validateDecisionParams` — the same disk-free checks `applyDecision` runs), then applies in order, each its own journal transaction; a failure never rolls back a sibling. Don't "optimize" it into a single multi-file transaction, and don't push or `onWritten()` per item (both happen once at the end). `dryRun` reuses the real path with the stage's `probe` flag — add a pre-check to the apply path (or `validateDecisionParams`) and the dry run gets it for free; add it anywhere else and they drift. The apply loop has a wall-clock budget under the manifest lock's 15 s waiter timeout (`BATCH_TIME_BUDGET_MS`); the rest comes back `BATCH_TIME_BUDGET` for the caller to resubmit — never before the first attempt. Automatic rules ARE this batch (`methodOverride: "automatic"` + a re-check `guard`); keep the guard's checks in sync with what `auto_ready` promised.
- **`rewrite_link`/`unlink` read the section live and touch only the effective contributor.** A broken link's `revision` fingerprints its target, not the section text, so two links in one section share a revision; the live read (`readLiveSection`) is what lets the second rewrite in a batch see the first. Never switch it to the projection's copy. `create_stub` is the one creating writer in the engine (`stageFileCreationTransaction`: exclusive placement, rollback by unlink, recovery unlinks a `created` target); the HTTP file routes still never create.
- **Only a broken-link rule may wildcard `conceptType`/`key`, and never `target`.** The exact target is what still pins a wildcard broken-link rule; a `section_content`/`frontmatter_value` rule with `*` would be pinned by kind and sources alone, and one flip to automatic would overwrite every disagreement between two layers — `validateRule` refuses both that and a wildcard target (the auto-acknowledge-a-typo failure the target pin exists to prevent). A wildcard rule and an exact rule that disagree are a `ruleConflict` (automation off) — do not invent a specificity order.
- **Never push the live layer while holding the manifest lock.** A push is up to three 90 s network calls; under the lock it times out every concurrent control-plane write and, past the stale threshold, lets another process steal the lock. `commitDecisionWrite` returns a `git.pushRoot` marker instead of pushing; `decide`/`decideBatch`/`runAutomaticRules` and the legacy conflict route call `pushAfterUnlock` once the lock is released. A host driving `applyDecision` under its own lock must do the same. Likewise every byte a locked git mutation changes — including startup recovery's restore of a live-layer target — happens *inside* `commitPathsWithMutation`'s `mutate`, whose rollback runs under the same repo lock even when the mutation itself throws.
...
- **Indexing passes queue, they don't all start at once** (`pumpIndexQueue`, `maxConcurrentIndexing`, default 4), and memory pressure can hold them back entirely. The queue bounds when a pass may *start*, not how long one stays alive — and foreign MCP servers may ignore protocol cancellation, so their own computation can outlive a cancelled caller. GitHub requests and MCP waits now receive the job signal. Details in [notes/index-queue-and-memory.md](docs/architecture/notes/index-queue-and-memory.md).
...
- **Cascade order is a reorder op, not arithmetic on `level`.** `PUT /api/sources/order` (`reorderSources` + pure `assignCascadeLevels` in `control/sources.mjs`) reassigns levels for the whole default profile — a permutation of the existing distinct levels when there are enough (3/2/0 stays {3,2,0}), N..1 otherwise — and `POST /api/sources` takes `position` (1 = top) as the alternative to `level`. Re-leveling never re-indexes (`level` is a presentation field, outside the index identity), and every level move on a `pack:` layer must go through `syncPackAssignmentLevel` or the strict write refuses on `pack-layer-drift`. The engine's precedence stays "higher `level` wins"; ranks are how the console shows it.
- **Index passes are incremental for local layers.** The walk fingerprints every document (`listEntries`: rel/ext/size/mtimeMs, plus the git `authoredDate` for `okf-local` — a commit changes a document's date without changing the document) and `snapshotSource` carries unchanged concepts forward as the same object — one edit costs one read; a pass that changed nothing returns the previous snapshot object so memos stay warm. "Nothing changed" is `canReuseSnapshot`, and it deliberately refuses when anything carried from the retry seed: that content is newer than the served snapshot even though the pass read nothing. Token counts persist across restarts (`token-count-cache.mjs`, `.cache/index/` beside the manifest). A transiently failed pass (timeout, EMFILE) retries on a bounded backoff and RESUMES from its carry seed; `?wait=` never waits on a parked retry. Search prefers `search-store.mjs`, a SQLite-backed BM25F posting store (`.cache/index/search.v1.<profileId>.sqlite`, beside the manifest) differential-tested bit-identical to `searchConcepts` and to the in-memory `search-index.mjs` it supersedes when `node:sqlite` is unavailable — which remains the scorer of record for the eval either way. Both the HTTP service and stdio MCP (`retained-search.mjs`) use it: a restarted process answers its first search from disk instead of re-parsing the corpus, and no per-document term map lives in the JS heap — `retained-search.mjs` asks the store which ids it lacks a fingerprint for (`store.pending(view)`) and loads only those, retaining just tiny per-layer fileMeta maps between queries, never parsed concepts. See [notes/retained-mcp-retrieval.md](docs/architecture/notes/retained-mcp-retrieval.md).
