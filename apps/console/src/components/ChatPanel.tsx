...
function ChatPanelInner({ keyboardSuspended = false, onConnectAgent, onClose }: { keyboardSuspended?: boolean; onConnectAgent?: () => void; onClose: () => void }) {
  ...
  }, [canAsk, docked, keyboardSuspended, onClose])
      ...
      <aside ref={panelRef} className="cc-ask-panel" role={docked ? 'complementary' : 'dialog'} aria-modal={docked ? undefined : 'true'} aria-label="Ask ContextCake">
        <header style={css('display:flex; align-items:center; gap:11px; padding:16px 18px; border-bottom:1px solid #D8D6CC;')}>
          ...
          <div style={css('flex:1; line-height:1.2;')}>
            ...
            <div style={css('font-size:11px; color:#8A8A82;')}>{subtitle}</div>
          </div>
          {onConnectAgent && mode === 'demo' && (
            <button
              className="cc-h-eae"
              onClick={connectAgent}
              title="Choose an AI client and connect ContextCake"
        ...
        <div ref={scrollRef} style={css('flex:1; overflow-y:auto; padding:18px; display:flex; flex-direction:column; gap:16px;')}>
          {mode === 'demo' && chatMessages.length === 0 && (
            <div className="cc-ui-empty">
              <strong>Try the demo cascade</strong>
              <p>These are sample answers, so you can see how citations and layer precedence work before connecting your own sources.</p>
            </div>
          )}
            ...
            return (
              <div key={i} style={css(`display:flex; ${isUser ? 'justify-content:flex-end;' : ''}`)}>
                <div style={css(`max-width:88%; padding:12px 14px; border-radius:13px; ${isUser ? 'background:#1E6B64; border-bottom-right-radius:4px;' : `background:#FFFFFF; border:1px solid ${C.line}; border-bottom-left-radius:4px;`}`)}>
                  ...
                  {m.canned && (
                    <div style={css(`margin-top:9px; padding-top:9px; border-top:1px solid #EAE7DD; font-family:${MONO}; font-size:10.5px; color:#8A8A82; letter-spacing:0.01em;`)}>Sample answer from the demo cascade</div>
                  )}
                </div>
              </div>
            )
