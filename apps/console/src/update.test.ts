...
describe('checkForUpdate', () => {
  ...
  it('returns update info when the newest coordinated app release is newer', async () => {
    mockReleases([
      { tag_name: 'app-v1.2.0', html_url: 'https://github.com/ContextCake/context-cake/releases/tag/app-v1.2.0' },
    ])

    const result = await checkForUpdate('1.1.0')
    expect(result).toEqual({ latest: '1.2.0', url: 'https://github.com/ContextCake/context-cake/releases/tag/app-v1.2.0' })
  })

  it('ignores engine v* releases — only the app-v* namespace counts', async () => {
    // Engine released more recently; the coordinated app release is older in the list.
    mockReleases([
      { tag_name: 'v9.0.0', html_url: 'https://example.com/v9.0.0' },
      { tag_name: 'app-v1.2.0', html_url: 'https://example.com/app-v1.2.0' },
    ])

    const result = await checkForUpdate('1.0.0')
    expect(result).toEqual({ latest: '1.2.0', url: 'https://example.com/app-v1.2.0' })
  })
  ...
  it('skips draft and prerelease app releases', async () => {
    mockReleases([
      { tag_name: 'app-v3.0.0', html_url: 'https://example.com/3', draft: true },
      { tag_name: 'app-v2.0.0', html_url: 'https://example.com/2', prerelease: true },
      { tag_name: 'app-v1.2.0', html_url: 'https://example.com/1' },
    ])
  ...
  it('returns null when the newest app release equals the current version', async () => {
    mockReleases([{ tag_name: 'app-v1.0.0', html_url: 'https://example.com' }])

    await expect(checkForUpdate('1.0.0')).resolves.toBeNull()
  })

  it('returns null when the newest app release is older', async () => {
    mockReleases([{ tag_name: 'app-v0.9.0', html_url: 'https://example.com' }])
  ...
  it('compares multi-segment versions numerically, not lexically', async () => {
    ...
    mockReleases([{ tag_name: 'app-v1.10.0', html_url: 'https://example.com' }])
  ...
  it('falls back to a canonical release URL when html_url is missing', async () => {
    mockReleases([{ tag_name: 'app-v1.2.0' }])

    const result = await checkForUpdate('1.0.0')
    expect(result?.url).toBe('https://github.com/ContextCake/context-cake/releases/tag/app-v1.2.0')
  })
  ...
  it('session-caches the result — only fetches once across repeated calls', async () => {
    mockReleases([{ tag_name: 'app-v2.0.0', html_url: 'https://example.com' }])
