...
// ---- Team sync: live captures + reuse metrics (team-activity.json) ----------
// Generated by team-activity.mjs; the panel stays hidden when the file is
// absent (file:// or team sync not in use).
loadTeamActivity();

async function loadTeamActivity() {
  try {
    const response = await fetch("./team-activity.json", { cache: "no-store" });
    if (!response.ok) return;
    renderTeamActivity(await response.json());
  } catch (error) {
    // keep the panel hidden
  }
}

function renderTeamActivity(data) {
  const panel = document.querySelector("#teamActivity");
  if (!panel || !data || !data.metrics) return;
  panel.hidden = false;

  document.querySelector("#activityGeneratedAt").textContent = `Generated ${formatTimestamp(data.generatedAt)}`;
  document.querySelector("#metricCrossBrain").textContent = data.metrics.crossBrainHits ?? 0;
  document.querySelector("#metricConfirmed").textContent = data.metrics.reviewThroughput?.confirmed ?? 0;
  document.querySelector("#metricPromoted").textContent = data.metrics.reviewThroughput?.promoted ?? 0;
  const reuse = data.metrics.medianTimeToFirstReuseHours;
  document.querySelector("#metricReuse").textContent = reuse == null ? "—" : `${reuse}h`;

  const feed = document.querySelector("#activityFeed");
  feed.innerHTML = (data.feed ?? [])
    .slice(0, 20)
    .map((item) => `
      <article class="signal-card${item.archived ? " signal-card--muted" : ""}">
        <header>
          <span class="badge">${escapeHtml(item.kind ?? "capture")}</span>
          <strong>${escapeHtml(item.title ?? item.id)}</strong>
        </header>
        <p>${escapeHtml(item.author ?? "unknown")} · ${formatTimestamp(item.capturedAt)}${item.archived ? " · archived" : ""} · ${escapeHtml(item.status ?? "")}</p>
      </article>`)
    .join("");

  const pending = document.querySelector("#pendingPromotions");
  const rows = data.pendingPromotions ?? [];
  pending.innerHTML = rows.length === 0 ? "" : `
    <p class="eyebrow">Pending promotions</p>
    ${rows.map((r) => `<p>${escapeHtml(r.promotedFrom ?? "?")} → <strong>${escapeHtml(r.promoteTo ?? "?")}</strong></p>`).join("")}`;
}
