...
`packages/core/fixtures/context-policy.json`:
...
node ingest.mjs --events packages/core/fixtures/mock-events.json --repos packages/core/fixtures/repos.json --out apps/control-surface/signals.json
```

Flags: `--events <file>` (default `packages/core/fixtures/mock-events.json`), `--repos <file>`
(default `packages/core/fixtures/repos.json`, used to attribute an owner per repo), `--out <file>`
(default `apps/control-surface/signals.json`), `--policy <file>` (default
`packages/core/fixtures/context-policy.json`), `--demo` (uses the bundled mock events), `--help`.
...
`apps/control-surface/signals.json` is generated output — it's gitignored. Serve
...
of the cascade — i.e. not the personal layer, whatever its level — or the top
layer if it's the only one available),
