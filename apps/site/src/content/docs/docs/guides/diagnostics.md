...
`contextcake` on `PATH` with its version, and local collector availability through
...
and are marked not probed. It does not read the running app's private observation
history.
