...
`examples/mock-mcp-source/server.mjs` is a deliberately non-OKF mock graph
...
{ "layers": [
  ...
  { "name": "company", "level": 0, "source": "mcp", "command": "node", "args": ["./examples/mock-mcp-source/server.mjs"] }
] }
```
