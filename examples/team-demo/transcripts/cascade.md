...
**Context source:** `examples/team-demo/manifests/full.json` — Personal (L3) > Team (L2) > Company (L0) cascade

**Expected shape:** a Scala + Spark Structured Streaming scaffold (sbt) that fits the team's repo, applies the inherited company secrets/auth + security conventions, and notes the company conflict.
...
- **Language conflict** ← Company (L0, 2026-06-01): Spring Boot with Java 21 remains the org standard; re-confirmation may be required.
- **Enforcement** ← Company (L0, inherited): Spring Boot/Java 21 conformance check in company CI.
