...
//   node ingest.mjs --demo            # uses bundled fixture events
//
...
const defaultEventsPath = path.join(packageRoot, "fixtures", "mock-events.json");
const defaultReposPath = path.join(packageRoot, "fixtures", "repos.json");
const defaultOutPath = path.join(repoRoot, "apps", "control-surface", "signals.json");
...
signals.json (signals + per-repo coverage). Defaults: --events
packages/core/fixtures/mock-events.json, --repos packages/core/fixtures/repos.json,
--out apps/control-surface/signals.json.
`);
}
