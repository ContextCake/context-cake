...
- [x] WHEN three distinct discrepancies receive the same structural manual decision THE SYSTEM SHALL offer an evidence-backed local rule suggestion; a broken-link rule's `conceptType` and `key` MAY generalize to the wildcard `*` only when the evidence spans at least two distinct (conceptType, key) pairs (offered as a separate `generalized: true` suggestion) — no other kind may use the wildcard, since only the pinned target keeps such a rule narrow — a broken-link rule SHALL always pin its exact `target` (never `*`), and a broken-link rule MAY carry a `rewrite_link { newTarget }` action; unlink and create_stub decisions are never evidence.
