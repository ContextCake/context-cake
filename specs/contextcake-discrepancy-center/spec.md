...
- [x] WHEN a client posts several decisions at once (`POST /api/discrepancy-decisions/batch`, at most 500, else 413) THE SYSTEM SHALL take one manifest lock and one settled projection (409 `COVERAGE_INCOMPLETE` for the whole batch while indexing), validate every item against that projection before applying any (unknown → `NOT_OPEN`, revision mismatch → `STALE`, repeated id → `DUPLICATE`, malformed → 400), then apply the valid ones in order as separate journal transactions — continuing past a failure unless `stopOnError`, never rolling back one item because another failed — and answer per-item results with `applied`/`failed` counts, one `onWritten`, one push of the live layer (`git: { layer, commits, pushed, queued }`, N commits for N content changes), and the rule suggestions the new decisions support; WHEN `dryRun` is set THE SYSTEM SHALL run every item's pre-checks and answer `wouldWrite: [{ layer, path, created? }]` without touching a file, the decision log, or git.
