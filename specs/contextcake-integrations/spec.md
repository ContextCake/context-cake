...
| `github` | `<owner>/<repo>/<path minus ext>` within the layer | same plain-markdown rules as `files` | latest commit author date for the file (cached; unavailable history stays undated, never committer date or repo `pushed_at`) | GitHub App **device flow**, `contents:read` |
